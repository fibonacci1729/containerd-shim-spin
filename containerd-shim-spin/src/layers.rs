...
fn locked_app_from_layers(layers: &[WasmLayer]) -> anyhow::Result<(usize, LockedApp)> {
    let (parent_idx, spin_layer) = find_spin_app_layer(layers).ok_or_else(|| anyhow::anyhow!("No Spin layer found"))?;
...
fn find_spin_app_layer(layers: &[WasmLayer]) -> Option<(usize, WasmLayer)> {
    for (i, layer) in layers.iter().enumerate() {
        match layer.config.media_type() {
            oci_spec::image::MediaType::Other(name) if name == spin_oci::client::SPIN_APPLICATION_MEDIA_TYPE => {
