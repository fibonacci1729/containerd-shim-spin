...
use containerd_shim_wasm::{
    container::{Engine, RuntimeContext, Stdio, PrecompiledLayer},
    sandbox::WasmLayer,
    version,
};
use futures::future;
...
use crate::{
    constants, layers, source::Source, trigger::{
...
impl Engine for SpinEngine {
    ...
    fn precompile(&self, layers: &[WasmLayer]) -> Result<Vec<PrecompiledLayer>> {
        Runtime::new()
            .context("failed to create runtime")?
            .block_on(layers::compose_and_precompile(&self.wasmtime_engine, layers))
    }
